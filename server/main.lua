...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "bcso" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
        if args[2] == "driver" or args[2] == "weapon" or args[2] == "business" or args[2] == "permit" then
...
QBCore.Commands.Add('fine', Lang:t("commands.fine"), {{name = 'id', help = Lang:t('info.player_id')}, {name = 'amount', help = Lang:t('info.amount')}}, false, function(source, args)
    ...
    if biller.PlayerData.job.name == "police" or biller.PlayerData.job.name == "bcso" then
            ...
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    if billed.Functions.RemoveMoney('bank', amount, "paid-fine") then
                        TriggerClientEvent('QBCore:Notify', source, 'Fine has been issued succesfully', 'success')
                        TriggerClientEvent('QBCore:Notify', billed.PlayerData.source, 'automatically recovered the fines owed..')
                        exports['qb-managementbusiness']:AddMoney(biller.PlayerData.job.name, amount)
        ...
        TriggerClientEvent('QBCore:Notify', source, 'On duty officer only', 'error')
    end
end)
...
QBCore.Commands.Add("depot", Lang:t("commands.depot"), {{name = "price", help = Lang:t('info.impound_price')}}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty or
    Player.PlayerData.job.name == "ambulance" and Player.PlayerData.job.onduty or
    Player.PlayerData.job.name == "bcso" and Player.PlayerData.job.onduty
    then
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer then
            ...
            exports['qb-managementbusiness']:AddMoney("police", price)
            TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("info.bill_received", {bill = price}))
        end
    end
end)

QBCore.Commands.Add("bill", Lang:t("commands.bill"), {{name = 'id', help = Lang:t('info.player_id')}, {name = 'amount', help = Lang:t('info.amount')}}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty or
    Player.PlayerData.job.name == "bcso" and Player.PlayerData.job.onduty then
        TriggerClientEvent("police:client:BillCommand", src)
    else
        TriggerClientEvent('QBCore:Notify', source, 'On duty officer only', 'error')
    end
end)
...
RegisterNetEvent('police:server:JailPlayer', function(playerId, time)
    ...
    local playerCoords = GetEntityCoords(playerPed)
    local targetCoords = GetEntityCoords(targetPed)
    if #(playerCoords - targetCoords) > 2.5 then end
...
RegisterNetEvent('police:server:removeItem', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('spawnstrips:client:spawnSpike', source)
end)