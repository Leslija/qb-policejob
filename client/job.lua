    ...
    local totalFuel = exports[Config.FuelScript]:GetFuel(vehicle)
        if vehicle ~= 0 and vehicle then
            ...
            if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
                QBCore.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 2000, false, true, {
...
RegisterNetEvent('qb-police:client:openArmoury', function()
    ...
    TriggerServerEvent('qb-log:server:CreateLog', 'pdarmory', 'Armoury Shop (Police)', 'blue', ('**Officer Name:** %s | **Citizen ID:** %s\n **Department:** %s | **Grade:** %s\n **Info:** Officer Accessed Armoury'):format(fullname, cid, PlayerData.job.label, PlayerData.job.grade.level))
end)
            ...
            if inImpound and PlayerJob.name == "police" or PlayerJob.name == "bcso" then
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        ...
                        local totalFuel = exports[Config.FuelScript]:GetFuel(veh)
...
CreateThread(function()
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            inImpound = true
            if onDuty and PlayerJob.name == "police" or PlayerJob.name == "bcso" then
            if IsPedInAnyVehicle(PlayerPedId(), false) then
                     exports['qb-core']:DrawText(Lang:t('info.impound_veh'), 'info')
                else
                    local currentSelection = 0
                    CreateThread(function()
                        while inImpound  do
                            Citizen.Wait(1)
                            exports['qb-core']:DrawText('[E] Police impound', 'left')
